...
This opens an interactive menu — **no command-line flags needed**:

```
  1.  Complete Installation / Re-configure
      Verify prerequisites, wallets, LLMs, and API connections

  2.  Autonomous Loop (Non-Stop 24/7)
      Runs forever: scan → solve → auto-submit PR / content → next task
      Stops only on Ctrl+C or when wallet receives first earning

  3.  Single Autonomous Cycle
      One bounty only: scan → select → solve → auto-submit → done

  4.  Interactive Mode (With Approval Gate)
      Shows proposal first, waits for your GO before doing any work

  5.  Exit
```

**First run?** Choose **`1`** to install and configure, then **`2`** for the relentless non-stop earning loop.
